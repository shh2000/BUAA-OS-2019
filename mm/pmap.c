...
page_init(void)
{
     ...
     * filed to 1) */
        int i;
        int npage_used=PADDR(freemem)/BY2PG;
        for(i=0;i<npage_used;i++){
                pages[i].pp_ref=1;
        }

    /* Step 4: Mark the other memory as free. */
        for(i=npage_used;i<npage;i++){
                pages[i].pp_ref=0;
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
     ...
     * When creating new page table, maybe out of memory. */
        if(((*pgdir_entryp&PTE_V)==0)&&create){
                ...
                ppage->pp_ref++;
                *pgdir_entryp=PADDR((Pde)page2kva(ppage)|PTE_R|PTE_V);
        }
