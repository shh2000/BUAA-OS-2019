...
int sys_mem_alloc(int sysno, u_int envid, u_int va, u_int perm)
{
        ...
        if((ret=page_insert(env->env_pgdir,ppage,va,perm))<0){
                printf("hahahahahaFuck83!!!Fuck83!!!\n");
                return ret;
        }
        return 0;
}
                                ...
                                u_int perm)
{
    ...
    //your code here
        ...
        if(srcva>=UTOP||dstva>=UTOP){
                printf("Fuck va!Fuck 83!\n");
                return -E_INVAL;
        }

        if((perm&PTE_V)==0){

                printf("Fuck 83 again!\n");
                return -E_INVAL;
        }
        if((ret=envid2env(srcid,&srcenv,0))<0){
                printf("Fuck 83!83 Fuck!\n");
                return ret;
        }
        ...
        if((ppage=page_lookup(srcenv->env_pgdir,round_srcva,&ppte))==0){
                printf("?why lookup can failed??Fuck!\n");
                return -E_INVAL;
        }
...
{
        int i,ret;
        if(!validdev(dev)||!validdev(dev+len-1)){
                return -E_INVAL;
        }
        for(i=0;i<len;i+=BY2PG){
                if((ret=va2pa(curenv->env_pgdir,va+i))<0){
                        return ret;
                }
        }
        bcopy((void*)va,(void*)(PHYSADDR_OFFSET+dev),len);
        return 0;
        // Your code here
}
...
int sys_read_dev(int sysno, u_int va, u_int dev, u_int len)
{
        ...
        bcopy((void*)(PHYSADDR_OFFSET+dev),(void*)va,len);
        return 0;
        // Your code here
}
